...
fuelWithFuelForFuel :: Integer -> Integer
fuelWithFuelForFuel = sum . List.takeWhile (>0) . List.drop 1 . List.iterate fuel

parseMass :: Parser (Maybe Integer)
parseMass = fmap pure L.decimal <|> (eof *> pure Nothing)
