...
type PhaseSetting = InterpreterInput
type Setting = [PhaseSetting]
...
solution1 = do
  program <- parseFile "input/Day7-1.txt" parseIntCodes <&> runCode
  pure $ List.maximumMaybe $ map (snd . program) $ List.permutations [0,1,2,3,4]

data FeedbackLoop m = FeedbackLoop
  { amplifiers           :: Vector.Vector (PhaseSetting, CPU m)
  , amplifierEvaluations :: Int
  }
...
runFeedbackLoopOnce
  :: PrimMonad m
  => Members '[State (FeedbackLoop m), Embed m] r
  => InterpreterInput
  -> Sem r (Maybe InterpreterOutput)
runFeedbackLoopOnce (InterpreterInput firstInput) =
  go (Just firstInput) =<< gets (Vector.toList . amplifiers)
    where
...
solution2 = do
  program <- parseFile "input/Day7-1.txt" parseIntCodes <&> runFeedbackLoop
  results <- mapM program $ List.permutations [5,6,7,8,9]
  pure $ List.maximumMaybe =<< sequenceA results

testCode :: [Int]
testCode = [3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26
